...
function Ecosystem() {
  ...
  // Cards fade up in place instead of flying from the hand below this width
  // — see the isCompact branch in the effect below. Non-reactive by design,
  // same convention as the `reduced` (prefers-reduced-motion) check further
  // down: evaluated once at mount, no resize listener. 768 must stay in
  ...
  useEffect(() => {
    ...
    // .hand never paints when isCompact — decoding its texture would be
    // pure waste on those viewports.
    const urls = [
      ...(isCompact ? [] : [cardTexture]),
      ...CONTENT.map(({ asset }) => asset).filter(Boolean),
    ]
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (isCompact) {
        ...
        // motion preference, so every compact visitor gets this treatment
